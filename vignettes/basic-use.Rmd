...
To download the data, we pass the catalog df to the `fetch_data` function.  Data are nested in a list-column, and since there is only one dataset to download, we can `unnest` in the same pipe.  
...
The agency names returned will depend on matches to the string supplied. To see all agencies cataloged, simply use `get_agencies` with the default argument:  
